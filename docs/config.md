...
- Borders are on by default, as in Omarchy ([borders.md](borders.md)): a ring in the
  ...
  by default; and `warning`, the color a window flashes for 0.3 s when its app refuses
  its tile, macOS's system red when left out or `true`. `warning = false` or a
  transparent `warning` turns the flash off. Colors are `#rrggbb`, or `#rrggbbaa` with the alpha last. The
