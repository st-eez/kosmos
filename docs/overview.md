...
| --- | --- | --- |
| Main actor | The model (inventory, workspaces, trees, focus intent), command execution, layout, hotkey dispatch, the bar snapshot, the display links that step slides and the bridged sends of a slide (below) | AX calls, any other bridged Space operation, waiting on another process, file syncs, process launches |
