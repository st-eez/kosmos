...
- `focus` in a direction goes up the tree to the nearest container along the direction
  ...
  - Each floating window stands in the container of the tile under its center, just
    ...
    `focus right` reaches it, and again reaches the right tile. With no tiles, the
    floating windows stand first in the root. Windows at one place go in the order of
    their centers along the container, and stacking plays no part.
  ...
  - Focusing a floating window raises it over the tiles it overlaps through the request
    every focus makes (section 5.4): AXRaise inside the front app, and the key record,
    then the worker's AXRaise, for another app. A keyboard focus change centers the
    pointer on a floating window as on a tile (section 5.11).
...
- Left out:
  ...
  - Keeping floating windows over a tile the pointer enters. Inside the front app only
    ...
    app's, or reorder only a background app's own windows. `focus` in a direction reaches a
    floating window a tile covers (section 5.5). If it bothers in practice, the
