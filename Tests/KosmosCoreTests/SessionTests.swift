...
@Test func aWindowOnScreenThatARevealedWorkspaceTakesInEntersIt() {
    ...
    #expect(probe.entering(show: follow.show, hide: follow.hide, frames: follow.frames.keys, concealed: concealed) == [2])
    ...
    #expect(probe.entering(show: empty.show, hide: empty.hide, frames: empty.frames.keys, concealed: concealed).isEmpty)
    probe = s
    let stay = probe.perform(.moveNodeToWorkspace(.named("2"), focusFollowsWindow: false, window: 2))!
    #expect(probe.entering(show: stay.show, hide: stay.hide, frames: stay.frames.keys, concealed: concealed).isEmpty)
    // A plain switch reveals only concealed windows.
    probe = s
    let plain = probe.perform(.workspace(.named("2")))!
    #expect(probe.entering(show: plain.show, hide: plain.hide, frames: plain.frames.keys, concealed: concealed).isEmpty)
    // A rule window opened on screen for workspace 2 and followed there.
    _ = s.add(4, to: "2")
    let rule = s.follow(4)
    #expect(s.entering(show: rule.show, hide: rule.hide, frames: rule.frames.keys, concealed: concealed) == [4])
}
