...
@Test func anEchoClearsTheCount() {
    ...
    _ = misses.willRequest(5, pid: 1, at: 50)
...
@Test func aLateEchoAfterAStaleActivationReadIsAHit() {
    ...
    let tripped = misses.willRequest(5, pid: 1, at: 200)
    #expect(!tripped)
    #expect(misses.inARow == 0)
}
...
@Test func echoesArrivingAfterTheNextRequestKeepTheCountDown() {
    ...
    for request in 1..<20 {
        ...
        tripped = misses.willRequest(window, pid: 1, at: request * 10) || tripped
        misses.reported(.window(window - 1), pid: 1, receivedAt: request * 10 + 1, echo: true)
        misses.reported(.window(window - 1), pid: 1, receivedAt: request * 10 + 2, echo: false)
    }
    #expect(!tripped)
}
...
@Test func aReportOfTheRequestedWindowIsNeverAMiss() {
    ...
    var tripped = false
    for request in 0..<10 {
        tripped = misses.willRequest(5, pid: 1, at: request * 10) || tripped
        misses.reported(.window(5), pid: 1, receivedAt: request * 10 + 1, echo: false)
    }
    tripped = misses.willRequest(5, pid: 1, at: 100) || tripped
    #expect(!tripped)
    #expect(misses.inARow == 0)
}
