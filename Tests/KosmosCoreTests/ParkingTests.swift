...
@Test func staleRestoreKeepsEveryWindowAtOnePoint() {
    ...
    workspace.park(1)
    workspace.insert(3)
    workspace.squeeze(2, to: 3, in: screen, gaps: Gaps())
...
@Test func staleRestoreWithNoRoomSplitsTheRoomiestWindow() {
    ...
    workspace.park(1)
    workspace.insert(3)
    workspace.squeeze(2, to: 1, in: screen, gaps: Gaps())
    workspace.unpark([1], in: screen, gaps: Gaps())
...
@Test func aWindowClosedAndKeptTakesTheReasonHeardAfterItsLook() {
    ...
    // A hide heard after the look leaves it closed and kept, the ceiling in docs/tree.md.
    #expect(s.park([3], because: .appHidden).isEmpty)
    #expect(s.parkReason(of: 3) == .closedByApp)
