...
(* a window key already, then records and raises the window, which keys    *)
(* it (RaiseKeys); the app performs the raise later (WorkerLand). For a    *)
...
(* key window. Reports come as the implementation takes them. An app's     *)
(* focus notification reaches its observer callback some time after the   *)
(* change (NoteDelay). The activation read runs on the app's worker,       *)
...
(* the user's next input, and an app's callbacks run before its            *)
(* activation read. A notification from an app Kosmos activated waits for  *)
(* that activation's read (HoldNotes), and a notice records that its app   *)
(* had already lost the front to Kosmos's activation (NoticeCheck). A      *)
...
(* NoticeCheck or PostRaise, or on NoteFollows or ReassertTakes, a rule    *)
(* the spec had, for configs that fail as expected.                        *)
(***************************************************************************)
...
CONSTANTS
    ...
    PostRaiseRecord \* with SplitQueue, the worker records that raise just before it
...
ASSUME NoteFollows \in BOOLEAN /\ ReassertTakes \in BOOLEAN /\ PostRaise \in BOOLEAN /\ PostRaiseRecord \in BOOLEAN
...
Init ==
    /\ \E f \in WsWins(1) :
         s = [ start    |-> f,
               ...
               lastRaced |-> FALSE ]  \* ghost: the user keyed another window of an app between the
                                      \* post-record raise's check and its landing
    /\ history = <<>>
...
KeyChangeBy(t, w, at, user) ==
    ...
    ELSE LET a == AppOfX(w)
             ...
             u == [t EXCEPT !.osFocus = w, !.rekey = <<>>, !.ne = t.ne + 1,
...
\* user's (with ReassertTakes, so is one it reasserts over), and only an activation read
\* follows: a focus notification is a change inside an app, which reaches a hidden window
\* only if a switch concealed it after the change and before the notification's callback.
...
ObserveSplit(t, ev0) ==
    LET ev == [ev0 EXCEPT !.prev = t.seenKey]
        ...
        a == AppOfX(ev.w)
        tk == IF bg THEN t ELSE [t EXCEPT !.seenKey = ev.w, !.waiting = IF ev.prev = @ /\ ks = {} THEN NoWin ELSE @]
...
Observe(t, evs) ==
    IF evs = <<>> THEN t
    ELSE Observe(IF SplitQueue THEN ObserveSplit(t, Head(evs)) ELSE ObserveOne(t, Head(evs)), Tail(evs))
...
Raise(t, a, w) ==
    ...
    IN IF w \in t.gone THEN t
       ELSE IF AppOfX(t.osFocus) = a THEN (IF RaiseKeys THEN KeyChange(u, w, Len(history)) ELSE u)
...
FocusDecide ==
    ...
    IN /\ SplitQueue
       ...
       /\ s' = IF SplitRules = "record-at-call"
               THEN IF c.front \/ c.g # s.gen \/ c.w \in s.gone \/ AppOfX(s.osFocus) = a \/ ph = "raising" THEN t
                    ELSE IF PostRaise
...
\* the user keys another window of the app before it lands; with PostRaiseRecord the
\* worker records just before it.
WorkerPost(a) ==
    LET j == Head(s.wq[a])
    IN /\ SplitQueue
       /\ s.wq[a] # <<>>
       /\ j.st = "post"
       /\ s' = IF AppOfX(s.osFocus) # a \/ s.afocus[a] # j.w \/ j.w \in s.gone THEN Finish(s, a, j)
               ELSE [(IF PostRaiseRecord THEN Record(s, j.w, j.r) ELSE s) EXCEPT !.wq[a][1].st = "land"]
       /\ UNCHANGED history
...
WorkerNote(a) ==
    ...
    IN /\ SplitQueue
       ...
       /\ s' = [s EXCEPT !.wq[a] = Tail(@), !.clk = s.clk + 1,
                         !.evs = Append(@, Ev(j.w, FALSE, j.t, 0, s.hidden[j.w], NoWin, AppOfX(s.osFocus) # a,
                                              s.clk, FALSE))]
       /\ UNCHANGED history
...
WorkerAct(a) ==
    ...
    IN /\ SplitQueue
       ...
       /\ s' = [s EXCEPT !.wq[a] = Tail(@),
                         ...
                         !.evs = Append(@, Ev(w, TRUE, j.t, 0, j.hs[w], NoWin, AppOfX(s.osFocus) # a, j.ts, j.ko))]
       /\ UNCHANGED history
...
Worker(a) == WorkerStart(a) \/ WorkerRead(a) \/ WorkerRaise(a) \/ WorkerPost(a) \/ WorkerLand(a)
             \/ WorkerKey(a) \/ WorkerNote(a) \/ WorkerAct(a) \/ LateLand(a)
...
ObserverPost(a) ==
    ...
    IN /\ SplitQueue
       ...
       /\ s' = [s EXCEPT !.nq[a] = Tail(@), !.clk = s.clk + 1,
                         ...
                         !.lastLost = @ \/ (bg /\ n.o.user /\ n.o.lone /\ n.w \in s.lastWin)]
       /\ UNCHANGED history
...
ActNotice ==
    ...
    IN /\ SplitQueue
       /\ s.an # <<>>
       /\ s' = IF n.w = NoWin
               THEN [t EXCEPT !.evs = Append(@, Ev(NoWin, TRUE, n.t, 0, FALSE, NoWin, AppOfX(s.osFocus) # "kosmos",
...
Rekey ==
    /\ s.rekey # <<>>
    /\ s' = Top(KeyChange([s EXCEPT !.rekey = <<>>], s.rekey[1].w, s.rekey[1].t), s.rekey[1].w)
    /\ UNCHANGED history
...
\* The user keys another window of an app whose post-record raise has checked its target
\* and not landed yet (PostRaise without a record).
Races(w) == SplitQueue /\ w # NoWin /\ ~PostRaiseRecord
            /\ \E n \in 1..Len(s.wq[AppOf[w]]) : LET j == s.wq[AppOf[w]][n]
                                                 IN j.st = "land" /\ j.w # w /\ s.kr[j.r] = "sent"
    ...
    /\ \E out \in {{s.osFocus}, AppWins(s.osFocus) \ s.gone}, closed \in BOOLEAN, quiet \in BOOLEAN, late \in BOOLEAN :
       ...
       /\ \E v \in IF quiet THEN {NoWin} ELSE (Win \ (s.gone \cup out)) \cup {NoWin} :
            LET q == SeqOf(out)
                ...
                t1 == [Fresh(s) EXCEPT !.gone = @ \cup out, !.lag = @ \cup out,
            ...
            IN /\ s' = CASE quiet -> [t1 EXCEPT !.osFocus = NoWin]
                        ...
                        [] OTHER -> Top(KeyChange(t1, v, Len(history) + 1), v)
...
Return ==
    ...
    /\ \E w \in s.gone \ (s.closed \cup s.lag) :
         /\ s' = Top(KeyChangeBy([Fresh(s) EXCEPT !.gone = @ \ {w}, !.lastWin = {w}, !.goal[DisplayOf[WsOf[w]]] = WsOf[w],
...
\* claim, or one of the races DESIGN.md 5.4 lists hid what the user chose.
...
HonorsLastClick == Quiescent /\ s.lastWin # {} /\ ~s.lastAmb /\ ~s.lastMis /\ ~s.lastRaced => s.focus \in s.lastWin
