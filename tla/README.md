...
visible windows, uses Command-Tab, and closes, minimizes or hides the key window.
...
| `fallback-user` | all inputs; macOS re-keys after a hide | last activation wins | fails, expected | 2,653,833 |
| `mixed` | commands, reveal first | no mixed frame | fails, expected | 103 |
| `conceal-first` | commands, conceal first | no mixed frame, no blank frame | fails, expected | 72 |
| `leave` | all inputs, and the key window leaving | convergence, last command wins, last activation wins, keeps its workspace after a leave, recovery path | pass | 831,784 |
| `leave-settles` | as `leave` | every disturbance settles (liveness) | pass | 831,784 |
| `leave-follow` | as `leave`, following re-keys as before | keeps its workspace after a leave | fails, expected | 302,667 |

The first three expected failures record trade-offs, and `leave-follow` records the
behaviour this model replaced:
...
The key window leaves only after Kosmos has had the reports before it. Kosmos handles one
in milliseconds, far below the time a person needs to see a window become key and then
close, minimize or hide it. The departure and the report of the next key window still
