...
final class Controller {
    ...
    func run(_ arguments: [String], received: ContinuousClock.Instant) -> (code: Int32, text: String) {
        ...
        case .success(let command):
            ...
            if let plan = session.perform(command) { execute(plan, since: received, fromCommand: true, movePointer: mouseFollowsFocus) }
            return (0, "")
        }
    }
    ...
    private func decide(_ report: KeyReport, keyLeft: Departure) {
        ...
        case .adopt(let window):
            ...
            // brings the pointer along; a click happens inside the window, which leaves the
            // pointer where it is.
