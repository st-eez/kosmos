...
public enum DisplayIdentity {
    ...
    /// Each display has its own framebuffer, so two identical monitors read the serials of
    /// their own panels. Matching by EDID fields instead cannot tell them apart: Steve's twin
    /// VG279QE5A panels share one EDID UUID, and CGDisplaySerialNumber, the EDID's numeric
    /// serial, is zero on both. When CoreDisplay stops naming the framebuffer, this returns nil
    /// and serial matchers match nothing.
