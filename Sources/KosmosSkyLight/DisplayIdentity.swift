...
public enum DisplayIdentity {
    ...
    /// The IORegistry path of the framebuffer that drives the display, such as
    /// `IOService:/AppleARMPE/arm-io@10F00000/AppleH15IO/disp0@7C000000/IOMobileFramebufferShim`.
    public static func framebuffer(of display: CGDirectDisplayID) -> String? {
    ...
    /// and CoreDisplay names that framebuffer by its registry path. Each display has its own
    /// framebuffer, so two identical monitors read the serials of their own panels. Matching
    /// by EDID fields instead cannot tell them apart: Steve's twin VG279QE5A panels share one
    /// EDID UUID, and CGDisplaySerialNumber, the EDID's numeric serial, is zero on both. When
    /// CoreDisplay stops naming the framebuffer, this returns nil and serial matchers match
    /// nothing.
    public static func serial(of display: CGDirectDisplayID) -> String? {
        guard let path = framebuffer(of: display) else { return nil }
