...
default:
    print("usage: kosmos-probe barrier [cycles] | survive-kill | bar | destroyed-space | gone-space-recovery | fullscreen | departures | tabs [strip|keep] | reveal | displays | secure-input | ax-timeout | keying [rounds] | level [onscreen|opaque] | sweep <stub.app>...")
    exit(2)
}
...
/// A small app for the sweep probe, run from its own bundle so that macOS counts each stub
/// as a separate app. A running window manager leaves its windows alone: they sit above
/// level 0. Prints its window ids, and exits when its standard input closes, so it never
/// outlives the probe that started it.
@MainActor func showStub(index: Int, windows: Int) -> Never {
    let app = NSApplication.shared
    app.setActivationPolicy(.regular)
    let screen = NSScreen.main?.visibleFrame ?? NSRect(x: 0, y: 0, width: 1440, height: 900)
    var ids: [Int] = []
    for offset in 0..<windows {
        // Stacked with an offset.
        let frame = NSRect(x: screen.maxX - CGFloat(index + 1) * 180 + CGFloat(offset) * 40,
                           y: screen.minY + 20 + CGFloat(offset) * 30, width: 170, height: 90)
...
func launchStub(_ bundle: String, index: Int, windows: Int = 1) -> Stub {
    ...
    process.arguments = ["stub", String(index), String(windows)]
    let input = Pipe(), output = Pipe()
    process.standardInput = input
    process.standardOutput = output
    try! process.run()
