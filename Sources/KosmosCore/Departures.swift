...
public struct DepartureLog: Sendable {
    ...
    /// Whether the window left within `limit` before `now`, or nil when no departure is on
    /// record.
    public func left(_ window: WindowID, within limit: Duration, at now: ContinuousClock.Instant) -> Bool? {
        leftAt[window].map { now - $0 <= limit }
    }
}

/// A key window report held until Kosmos knows whether the window key before it left
/// (tla/Kosmos.tla, Hold). Each hold carries the number of its grace timer, so a timer of a
/// replaced hold decides nothing.
public struct HeldReport<Report: Sendable>: Sendable {
    public private(set) var report: Report?
    private var previous: WindowID = 0
    private var number = 0

    public init() {}

    /// Holds `report` in place of any held one, and returns the number of its grace timer.
    public mutating func hold(_ report: Report, previous: WindowID) -> Int {
        number += 1
        self.report = report
    ...
    /// A newer activation of a window ends the hold.
    ...
    /// `windows` departed: the report to decide, if the window key before it is one of them.
    public mutating func departed(_ windows: [WindowID]) -> Report? {
        guard windows.contains(previous) else { return nil }
        defer { report = nil }
        return report
    }
}
...
public enum DepartureFocus: Equatable, Sendable {
    ...
    /// - Parameters:
    ///   - refocus: a focus request found the focus already gone.
    ...
    public static func decide(focusLeft: Bool, refocus: Bool, key: KeyWindow?, departing: [WindowID],
                              left: (WindowID) -> Bool) -> DepartureFocus {
        guard focusLeft else { return .none }
        if refocus { return .now }
