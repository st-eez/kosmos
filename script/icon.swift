...
// An .icns whose artwork fills that mask is shown at full size; one that does not is shrunk
// onto a grey rounded square.
import CoreGraphics
import Foundation
import ImageIO
...
/// A rounded rectangle whose corners are superellipse quarters, `corner` across.
func superellipse(_ rect: CGRect, corner: CGFloat, exponent m: CGFloat = 2.78) -> CGPath {
    let s = min(corner, rect.width / 2, rect.height / 2)
...
final class Canvas {
    ...
    /// A linear gradient across the current clip, or across `clip`.
    func wash(_ colors: [CGColor], from: CGPoint, to: CGPoint, _ locations: [CGFloat]? = nil, clip: CGPath? = nil) {
        ctx.saveGState()
        if let clip { ctx.addPath(clip); ctx.clip() }
        ctx.drawLinearGradient(gradient(colors, locations), start: from, end: to, options: [])
        ctx.restoreGState()
    }
    func radial(_ center: CGPoint, _ radius: CGFloat, _ colors: [CGColor], _ locations: [CGFloat]? = nil, clip: CGPath? = nil) {
        ctx.saveGState()
        if let clip { ctx.addPath(clip); ctx.clip() }
...
func draw(_ c: Canvas) {
    ...
    c.clipped(bodyPath) {
        ...
        c.wash(hot, from: CGPoint(x: vx, y: 0), to: CGPoint(x: vx + gap / 2, y: 0), stops, clip: slot)
        c.wash(hot, from: CGPoint(x: vx, y: 0), to: CGPoint(x: vx - gap / 2, y: 0), stops, clip: slot)
        ...
        for half in [leftHalf, rightHalf] {
            c.clipped(half) {
                ...
                c.radial(CGPoint(x: 400, y: 640), 420, [rgb(0xffffff, 0.22), rgb(0xffffff, 0.05), rgb(0xffffff, 0)], [0, 0.5, 1], clip: sphere)
